...
if ($meal_period) {
  ...
  mysqli_stmt_bind_param($stmt_now, $now_types, ...$now_params);
...
<body>
  <div class="menu-page">
    ...
    <main class="menu-content">
      ...
      <!-- =================================================== -->
      <!-- SECTION 1: NOW SERVING MEAL PERIOD (ORDERABLE)      -->
      <!-- =================================================== -->
      <div style="margin:20px 0 10px;">
        ...
        <p style="font-size:13.5px; color:#6b7280; margin:0;">
          Items available to order right now for quick pickup!
        </p>
      </div>

      <?php if ($meal_period): ?>
        <div style="display:flex; align-items:center; gap:10px; margin:0 0 16px; padding:10px 16px; background:#f5f3ff; border:1px solid #ddd6fe; border-radius:10px; width:fit-content;">
          <span style="font-weight:700; color:#7047f2; font-size:14px;">
      ...
      <section class="food-list">
        ...
        <?php if (!$meal_period): ?>
          <div style="padding:40px 20px; text-align:center; color:#6b7280; width:100%;">
            ...
            <p style="font-size:13.5px;">Ordering for specific meal periods is available during:</p>
            <p style="font-size:13.5px; margin-top:6px;">
              🌅 Breakfast: 6:00 AM – 12:00 PM &nbsp;|&nbsp; 🍛 Lunch: 12:00 PM – 6:00 PM &nbsp;|&nbsp; 🌙 Dinner: 6:00 PM – 9:00 PM
            </p>
          </div>
        ...
        <?php else: ?>
          <?php while ($f = mysqli_fetch_assoc($now_serving_menu)): ?>
            <article class="food-card">
              <img src="../Images/<?= e($f['image']) ?>" alt="<?= e($f['food_name']) ?>" onerror="this.src='../Images/food.jpg'" />
      ...
      <!-- =================================================== -->
      <!-- SECTION 2: ALL MENU ITEMS (VIEW / BROWSING ONLY)    -->
      <!-- =================================================== -->
      <div style="margin:20px 0 10px;">
        <h2 style="font-size:20px; font-weight:700; color:#111827; margin:0 0 4px;">
          All Menu Items
      ...
      <section class="food-list">
        <?php if (mysqli_num_rows($all_menu) === 0): ?>
          <p style="padding:20px; color:#888;">No menu items found.</p>
        <?php else: ?>
          <?php while ($f = mysqli_fetch_assoc($all_menu)): ?>
            ...
            <article class="food-card">
              ...
              <?php else: ?>
                <button type="button" class="order-btn" disabled style="width:100%; background:#d1d5db !important; color:#6b7280 !important; cursor:not-allowed; border:none;">
                  Not Serving Now
                </button>
              <?php endif; ?>
            </article>

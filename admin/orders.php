...
// ── DATE FILTER (optional — blank means all dates) ─────────────
$date_filter = $_GET['date'] ?? '';
if ($date_filter !== '' && (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date_filter) || !strtotime($date_filter))) {
  $date_filter = '';
}

// ── FETCH orders (status + optional date, both via prepared statements) ──
$where  = [];
$types  = '';
$params = [];
    ...
    <div class="main-content">
      ...
      <div class="order-tabs">
        <?php foreach (['All', 'Pending', 'Processing', 'Ready', 'Completed'] as $tab): ?>
          <a href="orders.php?status=<?= $tab ?><?= $date_filter !== '' ? '&date=' . e($date_filter) : '' ?>" style="text-decoration:none;">
      ...
      <!-- Search + Date Filter -->
      <div class="orders-search-filter" style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
        <div class="orders-search-box">
          <img src="images/search.png" alt="Search">
          <input type="text" id="orderSearchInput" placeholder="Search Orders...">
        </div>

        <form method="GET" action="orders.php" style="display:flex; align-items:center; gap:8px;">
          ...
          <input
            ...
            style="padding:9px 12px; border-radius:8px; border:1px solid #d1d5db; font-size:13px; outline:none; cursor:pointer;">
          <?php if ($date_filter !== ''): ?>
            <a href="orders.php?status=<?= e($filter) ?>" style="font-size:12px; font-weight:bold; color:#7047f2; text-decoration:none; white-space:nowrap;">Clear date</a>
          <?php endif; ?>
        </form>
      </div>

      <?php if ($date_filter !== ''): ?>
        <p style="margin:10px 0 0; color:#6b7280; font-size:13.5px;text-align:center;">
          Showing orders for <strong><?= date('d M Y', strtotime($date_filter)) ?></strong>
        </p>
      <?php endif; ?>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
  ...
  $category_arr = $_POST['foodCategory']      ?? [];
  $category_arr = array_intersect($category_arr, ['Breakfast', 'Lunch', 'Dinner']); // whitelist
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit') {
  ...
  $category_arr = $_POST['editCategory']          ?? [];
  $category_arr = array_intersect($category_arr, ['Breakfast', 'Lunch', 'Dinner']); // whitelist
  $category     = implode(',', $category_arr);
...
<body data-flash-type="<?= e($flash_type) ?>" data-flash-msg="<?= e($msg) ?>">
      ...
      <div class="food-modal" id="foodModal">
        <div class="food-modal-box">
          ...
          <form method="POST" action="food_items.php" enctype="multipart/form-data">
            ...
            <label style="font-size:13px; color:#555; display:block; margin-bottom:6px; font-weight:bold;">Category (select one or more)</label>
            <div class="category-checkbox-group" style="display:flex; flex-wrap:wrap; gap:14px; margin-bottom:14px;">
              <label style="display:flex; align-items:center; gap:5px; font-size:13.5px; font-weight:normal;"><input type="checkbox" name="foodCategory[]" value="Breakfast"> Breakfast</label>
              <label style="display:flex; align-items:center; gap:5px; font-size:13.5px; font-weight:normal;"><input type="checkbox" name="foodCategory[]" value="Lunch"> Lunch</label>
              <label style="display:flex; align-items:center; gap:5px; font-size:13.5px; font-weight:normal;"><input type="checkbox" name="foodCategory[]" value="Dinner"> Dinner</label>
            </div>
      ...
      <div class="food-modal" id="editFoodModal">
        <div class="food-modal-box">
          ...
          <form method="POST" action="food_items.php" enctype="multipart/form-data">
            ...
            <label style="font-size:13px; color:#555; display:block; margin-bottom:6px; font-weight:bold;">Category (select one or more)</label>
            <div class="category-checkbox-group" id="editCategoryGroup" style="display:flex; flex-wrap:wrap; gap:14px; margin-bottom:14px;">
              <label style="display:flex; align-items:center; gap:5px; font-size:13.5px; font-weight:normal;"><input type="checkbox" name="editCategory[]" value="Breakfast" class="edit-cat-cb"> Breakfast</label>
              <label style="display:flex; align-items:center; gap:5px; font-size:13.5px; font-weight:normal;"><input type="checkbox" name="editCategory[]" value="Lunch" class="edit-cat-cb"> Lunch</label>
              <label style="display:flex; align-items:center; gap:5px; font-size:13.5px; font-weight:normal;"><input type="checkbox" name="editCategory[]" value="Dinner" class="edit-cat-cb"> Dinner</label>
            </div>
              ...
              <?php else: ?>
                <?php foreach ($food_rows as $f): ?>
                  ...
                  <tr>
                    ...
                    <td><?= e($f['food_name']) ?></td>
                    <td><?= e(str_replace(',', ', ', $f['category'])) ?></td>
  ...
  <script>
    ...
    function openEditModal(id, name, cat, price, stock, status, description) {
      ...
      // Check the relevant category checkboxes (cat is a comma-separated string, e.g. "Breakfast,Dinner")
      const selectedCats = cat ? cat.split(',').map(c => c.trim()) : [];
      document.querySelectorAll('.edit-cat-cb').forEach(function(cb) {
    ...
    // Require at least one category checkbox checked (Add form)
    document.querySelector('#foodModal form').addEventListener('submit', function(e) {
      const checked = this.querySelectorAll('input[name="foodCategory[]"]:checked');
      if (checked.length === 0) {
        e.preventDefault();
    ...
    // Require at least one category checkbox checked (Edit form)
    document.querySelector('#editFoodModal form').addEventListener('submit', function(e) {
      const checked = this.querySelectorAll('input[name="editCategory[]"]:checked');
      if (checked.length === 0) {
        e.preventDefault();
        alert('Please select at least one category.');
      }
